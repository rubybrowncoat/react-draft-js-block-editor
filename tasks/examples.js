...
module.exports = function (gulp, config) {
	...
	gulp.task('watch:examples', [
		...
		var watchSASS = [];
		if (config.example.scss && config.example.scss.length > 0) {
			config.example.scss.forEach(function(fileName) {
				watchSASS.push(config.example.src + '/' + fileName);
			});
		}

		if (config.component.scss && config.component.scss.path) {
			watchSASS.push(config.component.scss.path + '/**/*.scss');
		}
